...
public class Utility {
  ...
  public static String weather_paging(int totalRecord, int nowPage, int recordPerPage, String col, String word, String weather) {
    int pagePerBlock = 3; // 블럭당 페이지 수
    int totalPage = (int) (Math.ceil((double) totalRecord / recordPerPage)); // 전체 페이지, ceil: 올림
    int totalGrp = (int) (Math.ceil((double) totalPage / pagePerBlock));// 전체 그룹
    int nowGrp = (int) (Math.ceil((double) nowPage / pagePerBlock)); // 현재 그룹
...
//  }
  ...
  public static String spaging(int totalRecord, int nowPage, int recordPerPage, String col, String word, String url) {
    ...
    if (nowGrp >= 2) {
      str.append("<div><a href='" + url + "?col=" + col + "&word=" + word + "&nowPage=" + _nowPage
          + "'><i class=\"fa-solid fa-angle-left\"></i></A></div>");
    }
  ...
  public static String slpaging(int totalRecord, int nowPage, int recordPerPage, String col, String word, String url) {
	    int pagePerBlock = 3; // 블럭당 페이지 수
	    int totalPage = (int) (Math.ceil((double) totalRecord / recordPerPage)); // 전체 페이지, ceil: 올림
	    int totalGrp = (int) (Math.ceil((double) totalPage / pagePerBlock));// 전체 그룹
	    int nowGrp = (int) (Math.ceil((double) nowPage / pagePerBlock)); // 현재 그룹
	    ...
	    int _nowPage = (nowGrp - 1) * pagePerBlock; // 10개 이전 페이지로 이동
	    if (nowGrp >= 2) {
	      str.append("<li><a href='" + url + "?col=" + col + "&word=" + word + "&nowPage=" + _nowPage
	          + "'><i class=\"fa-solid fa-angle-left\"></i></a></li>");
	    }
